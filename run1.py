...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # 取最后一个时间步的输出
        out = self.fc(lstm_out[:, -1, :])  # 仅使用最后一个时间步的输出
        return out
...
def main(
    n_hours=1, num_rounds=5, hidden_size=64, num_layers=1
):  # 添加了 hidden_size 和 num_layers 参数
    # ========== 数据预处理 ==========
    # 加载数据
    ...
    for round_num in range(num_rounds):
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, targets in train_loader:
                # LSTM 需要输入形状为 (batch_size, seq_len, input_size)
...
if __name__ == "__main__":
    main(n_hours=96, num_rounds=5)  # 设置进行五轮实验
